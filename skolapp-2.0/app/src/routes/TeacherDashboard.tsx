...
import { useLocalQuizzes, validateQuizLegacy } from '../hooks/useLocalQuizzes';
...
export const TeacherDashboard: React.FC = () => {
  ...
  const { localQuizzes, addQuiz, createQuiz, merged } = useLocalQuizzes();
  const [title, setTitle] = useState('');
  const [question, setQuestion] = useState('');
  const [errors, setErrors] = useState<string[]>([]);
  const [touched, setTouched] = useState(false);
  const [saving, setSaving] = useState(false);
  const [useAdvancedCreator, setUseAdvancedCreator] = useState(false);
  const online = typeof navigator !== 'undefined' ? navigator.onLine : true;
  const currentErrors = validateQuizLegacy(title, question);
  ...
  return (
    <section>
      ...
      <div style={{ marginBottom: '2rem', display: 'flex', gap: '1rem', alignItems: 'center' }}>
        <Button 
          variant={!useAdvancedCreator ? 'primary' : 'secondary'}
          onClick={() => setUseAdvancedCreator(false)}
        >
          Snabb quiz (Legacy)
        </Button>
        <Button 
          variant={useAdvancedCreator ? 'primary' : 'secondary'}
          onClick={() => setUseAdvancedCreator(true)}
        >
          Avancerad quiz (MCQ, Sant/Falskt, Tidsgränser)
        </Button>
      </div>

      {useAdvancedCreator ? (
        <QuizCreator 
          onSave={(title, questions, description) => createQuiz(title, questions, description)}
          onCancel={() => setUseAdvancedCreator(false)}
        />
      ) : (
        /* Legacy Quick Quiz Creator */
        <form onSubmit={handleSubmit} aria-labelledby="create-quiz-h" style={{marginBottom: '1rem', border:'1px solid var(--border)', padding:'1rem', borderRadius:4}}>
          <h3 id="create-quiz-h">Skapa nytt quiz (snabb)</h3>
